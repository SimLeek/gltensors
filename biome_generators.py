...
def plains_gen(model, n, s, y, turbulence=1):
    for x in xrange(-n, n + 1, s):
        for z in xrange(-n, n + 1, s):
            ...
            '''for h in xrange(y - 2, top_h):
                if h == top_h-1:
                    model.add_block((x, h, z), 'grass', immediate=False)
                else:
                    model.add_block((x, h, z), 'dirt', immediate=False)'''
...
def cloud_layer_gen(model, n, s, y, size = 1, density=0, min_height=20, max_height=40, cloud_height=30, dissipation_rate=.1):
    for x in xrange(-n, n + 1, s):
        for z in xrange(-n, n + 1, s):
            noise = OpenSimplex()

            for h in xrange(min_height, max_height):
                if noise.noise3d(x / (10.0*size), z / (10.0*size), h/(3.0*size)) > density+ abs(h-cloud_height)*dissipation_rate:
